#!/usr/bin/env python3
"""Push reply-classification records to CRM via agentic-app API.

Reads JSON from stdin, routes to the appropriate CRM adapter,
and POSTs each record to CRM_URL/api/push.

Source tag: skill:reply-classification:v2.1.0
"""
...
SKILL_NAME = "reply-classification"
SKILL_VERSION = "2.1.0"
...
def push_record(record: dict) -> dict:
    """Push a single record to the CRM push API."""
    crm_url = get_crm_url()
    token = get_token()
